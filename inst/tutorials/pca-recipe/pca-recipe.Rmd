...
Now that we have an idea about variable loadings, let's talk about how we made this plot. Just like anything in life, we first need to get our data ready.  
...
Good job! You completed all the steps and applied dimensionality reduction to the `zoo` data set using the [recipes](https://recipes.tidymodels.org/) package from tidymodels! `r emo::ji("winner")` But before you start your victory lap, let's go over what we learned one last time.
...
Congratulations! You've completed the tutorial! It's time for your victory lap! `r emo::ji_glue(":victory::runner:")`
