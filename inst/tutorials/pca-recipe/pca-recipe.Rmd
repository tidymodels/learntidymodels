...
Alternatively, we could have accomplished scaling and centering in one single step. Take a look [this group of step functions](https://recipes.tidymodels.org/reference/index.html#section-step-functions-normalization) on the recipes reference page. See if you can answer the question below correctly:
...
##### You can see that these _intermediate values_ can be different for each step, but are always called `values` when extracted with the `tidy` method. You can find the definition of these _internmediate values_ under the *Value* section in the help page of the related step function. For example, take a look at [`step_scale` help document](https://recipes.tidymodels.org/reference/step_scale.html) and scroll down to see the *Value*.


Alternatively, we can use the `id` argument (the one we specifically provided for this step) and specify the `type` of _intermediate value_ we would like to extract.
...
Let's take a look at loadings we generated with the zoo data for the first four principle components and top six variables with largest variable loadings. Here, we plot the absolute values of the loadings to easily compare them, and color it by the direction of the loading (positive or negative):

```{r load-plot, echo=FALSE}
prin_comps <- c("PC1", "PC2", "PC3", "PC4")
num_loads <- 6

load_plot <- pca_loading %>%
  filter(component %in% prin_comps) %>%
  mutate(abs_load = abs(value),
         direction = value > 0) %>%
  group_by(component) %>%
  top_n(num_loads, abs_load) %>%
  ungroup()
...
load_plot %>%
  mutate(terms = tidytext::reorder_within(terms, abs_load, component)) %>%
  ggplot(aes(abs_load, terms, fill = value > 0)) +
  geom_col() +
...
Now that we have an idea about variable loadings, let's talk about how we make this plot. Just like anything in life, we first need to get our data ready for plotting.  

1. To keep our plot simple, we looked at the loadings for the first four principle components and top six variables with largest absolute loadings. For that, we assign the principal components we would like to plot to `prin_comps` and number of loadings you would like to show to `num_loads`. Feel free to change what goes into these variables to plot more or less information.  

1. We will add two new columns to `pca_loading` before plotting them:

    + Column `abs_load` will contain the absolute values of the loadings.
    + Column `direction` will inform us of their direction in relation to the principal component. 

1. Then we will use two functions from the [tidytext](https://github.com/juliasilge/tidytext) package: `reorder_within()` and `scale_y_reordered()` to order our bar plot neatly within facets.  

```{r pc-cont, exercise=TRUE, exercise.lines=30}
prin_comps <- c("PC1", "PC2", "PC3", "PC4")
num_loads <- 6
...
load_plot %>%
  mutate(terms = tidytext::reorder_within(terms, abs_load, component)) %>%
  ggplot(aes(abs_load, terms, fill = direction)) +
  geom_col() +
  facet_wrap(~component, scales = "free_y") +
  tidytext::scale_y_reordered() +
  scale_fill_manual(values = c("#372F60", "#CA225E")) +
...
Looks like increasing the number of UMAP components did not change our results that much. Try other hyperparameters and see if you can produce different plots. Just don't forget to modify the plot accordingly.
...
However, if you are only working with one data set you can simply use `juice` to apply your computations to the defined data in your recipe. For example, throughout this tutorial, we only worked with the `zoo` data and did not need to split our data into training or test sets. We could easily extract our principal components with `juice(pca_prep)`.
...
Great! Now, let's juice! `r emo::ji("juice")` 
...
Do you see any difference in the output between bake or juice? If you do it's probably your imagination, because there's no difference between the two. 
